...
router.delete("/experience/:exp_id", auth, async (req, res) => {
  try {
    ...
    if (removeIndex < 0) {
      return res.status(400).json({ msg: "해당하는 경력이 존재하지 않습니다. " });
    }
...
router.delete("/education/:edu_id", auth, async (req, res) => {
  try {
    ...
    if (removeIndex < 0) {
      return res.status(400).json({ msg: "해당하는 학력정보가 존재하지 않습니다. " });
    }
  ...
  try {
    const options = {
      uri: `https://api.github.com/users/${
        req.params.username
      }/repos?per_page=30&sort=created:asc&client_id=${config.get(
        "githubClientId"
